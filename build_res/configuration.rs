...
impl Configuration {
	...
	fn secretstore_config(&self) -> Result<SecretStoreConfiguration, String> {
		if self.secretstore_enabled() && !self.rpc_enabled() {
			return Err("secretstore requires configured (local or global) json rpc over http".into());
		}
		Ok(SecretStoreConfiguration {
			...
			self_http_rpc_interface: self.rpc_interface(),
			self_http_rpc_port: self.args.arg_ports_shift + self.args.arg_rpcport.unwrap_or(self.args.arg_jsonrpc_port)
		})
	}
