...
use key_server_cluster::message::{self, Message, DecryptionMessage, SchnorrSigningMessage, ConsensusMessageOfShareAdd, ShareAddMessage, ServersSetChangeMessage, ConsensusMessage, ConsensusMessageWithServersSet, EcdsaSigningMessage, DecryptionWithPayloadMessage, KeyVersionNegotiationMessage};
...
use key_server_cluster::decryption_session_with_payload::{
	SessionImpl as DecryptionSessionWithPayloadImpl,
	SessionCreationData as DecryptionSessionWithPayloadCreationData,
	SessionParams as DecryptionSessionWithPayloadParams
};
...
impl SessionCreatorCore {
	...
	pub fn new(config: &ClusterConfiguration, eth_client: Arc<dyn EthClient>) -> Self {
		SessionCreatorCore {
...
/// Decryption session creator with payload
pub struct DecryptionWithPayloadSessionCreator {
    /// Creator core.
    pub core: Arc<SessionCreatorCore>,
...
impl ClusterSessionCreator<DecryptionSessionWithPayloadImpl>
    for DecryptionWithPayloadSessionCreator
{
    /// is called when the slave receives a message that doesn't correspond to a running session
    fn creation_data_from_message(
        message: &Message,
    ) -> Result<Option<DecryptionSessionWithPayloadCreationData>, Error> {
		debug!("received message {}", message);
        match *message {
            Message::DecryptionWithPayload(DecryptionWithPayloadMessage::KeyVersionNegotiation(ref msg)) => match (&msg.signing_key, &msg.decryption_key) {
				(KeyVersionNegotiationMessage::RequestKeyVersions(_), KeyVersionNegotiationMessage::RequestKeyVersions(_)) => Ok(None),
				_ => Err(Error::InvalidMessage),
            },
            // Message::Decryption(DecryptionMessage::DecryptionSessionDelegation(ref message)) => {
            //     Ok(Some(message.requester.clone().into()))
            // }
            _ => Err(Error::InvalidMessage),
        }
    }
    ...
    /// is called
    fn make_error_message(sid: SessionIdWithSubSession, nonce: u64, err: Error) -> Message {
        message::Message::DecryptionWithPayload(message::DecryptionWithPayloadMessage::DecryptionSessionError(
            message::DecryptionSessionError {
                session: sid.id.into(),
                sub_session: sid.access_key.into(),
                session_nonce: nonce,
                error: err.into(),
    ...
    fn create(
        ...
        nonce: Option<u64>,
        id: SessionIdWithSubSession,
        creation_data: Option<DecryptionSessionWithPayloadCreationData>,
    ) -> Result<WaitableSession<DecryptionSessionWithPayloadImpl>, Error> {
        let decryption_key_share = self.core.read_key_share(&id.id)?;
		let decryption_key_share = decryption_key_share.ok_or(Error::DocumentKeyIsNotFound)?;
		let signing_key_share = self.core.read_key_share(&JOINT_SIGNATURE_KEY_ID)?;
		let signing_key_share = signing_key_share.ok_or(Error::SigningKeyIsNotFound)?;
        let nonce = self.core.check_session_nonce(&master, nonce)?;
		let params = DecryptionSessionWithPayloadParams {
			meta: SessionMeta {
				id: id.id,
				self_node_id: self.core.self_node_id.clone(),
				master_node_id: master,
				threshold: decryption_key_share.threshold,
				configured_nodes_count: cluster.configured_nodes_count(),
				connected_nodes_count: cluster.connected_nodes_count(),
			},
			access_key: id.access_key,
			...
			cluster: cluster,
			nonce: nonce,
			self_public: self.self_public.clone(),
			log_contract_address: self.log_contract_address.clone(),
			eth_client: self.core.eth_client.clone(),
		};
		let (session, oneshot) = match creation_data {
			Some(creation_data) => DecryptionSessionWithPayloadImpl::new_from_master(params, creation_data.requester, creation_data.payload)?,
			None => DecryptionSessionWithPayloadImpl::new_from_slave(params)?,
		};
        Ok(WaitableSession::new(session, oneshot))
    }
}
...
impl IntoSessionId<SessionId> for Message {
	fn into_session_id(&self) -> Result<SessionId, Error> {
		match *self {
			...
			Message::Decryption(_) | Message::DecryptionWithPayload(_) => Err(Error::InvalidMessage),
...
impl IntoSessionId<SessionIdWithSubSession> for Message {
	fn into_session_id(&self) -> Result<SessionIdWithSubSession, Error> {
		match *self {
			...
			Message::DecryptionWithPayload(ref message) => Ok(SessionIdWithSubSession::new(message.session_id().clone(), message.sub_session_id().clone())),
