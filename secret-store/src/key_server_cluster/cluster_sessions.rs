...
use key_server_cluster::decryption_session_with_payload::SessionImpl as DecryptionSessionWithPayloadImpl;
...
use key_server_cluster::cluster_sessions_creator::{GenerationSessionCreator, EncryptionSessionCreator, DecryptionSessionCreator,
	...
	EcdsaSigningSessionCreator, ClusterSessionCreator, DecryptionWithPayloadSessionCreator};
...
pub struct ClusterSessions {
	...
	/// decryption sessions that use a payload for authorization
	pub decryption_sessions_payload: ClusterSessionsContainer<DecryptionSessionWithPayloadImpl,DecryptionWithPayloadSessionCreator>,	
...
impl ClusterSessions {
	...
	pub fn new(config: &ClusterConfiguration, servers_set_change_session_creator_connector: Arc<dyn ServersSetChangeSessionCreatorConnector>, log_contract_address: Option<Address>, eth_client: Arc<dyn EthClient>) -> Self {
		let container_state = Arc::new(Mutex::new(ClusterSessionsContainerState::Idle));
		let creator_core = Arc::new(SessionCreatorCore::new(config, eth_client));
		ClusterSessions {
			...
			decryption_sessions_payload: ClusterSessionsContainer::new(DecryptionWithPayloadSessionCreator {
				core: creator_core.clone(),
				self_public: config.self_key_pair.public().clone(),
	...
	pub fn preserve_sessions(&mut self) {
		...
		self.decryption_sessions_payload.preserve_sessions = true;
	...
	pub fn stop_stalled_sessions(&self) {
		...
		self.decryption_sessions_payload.stop_stalled_sessions();
	...
	pub fn on_connection_timeout(&self, node_id: &NodeId) {
		...
		self.decryption_sessions_payload.on_connection_timeout(node_id);
...
mod tests {
	...
	pub fn make_cluster_sessions() -> ClusterSessions {
		...
		ClusterSessions::new(&config, Arc::new(SimpleServersSetChangeSessionCreatorConnector {
			admin_public: Some(Random.generate().public().clone()),
		}), Some(Address::default()), Arc::new(DummyEthClient::new()))
	}
