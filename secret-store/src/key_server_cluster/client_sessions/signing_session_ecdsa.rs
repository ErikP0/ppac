...
struct SessionCore {
	...
	/// if true, the jointly computed signature is broadcast to every node
	/// if false, the signature will only be available on the master node
	pub broadcast_signature: bool
}
...
impl SessionImpl {
	...
	pub fn state_str(&self) -> String {
		...
		let sig_nonce = data.sig_nonce_generation_session.as_ref().map_or("None".to_string(), |s| s.state_str());
		let inv_nonce = data.inv_nonce_generation_session.as_ref().map_or("None".to_string(), |s| s.state_str());
		let inv_zero = data.inv_zero_generation_session.as_ref().map_or("None".to_string(), |s| s.state_str());
		if data.state == SessionState::NoncesGenerating {
			format!("{:?}[sig_nonce={}, inv_nonce={}, inv_zero={}]", data.state, sig_nonce, inv_nonce, inv_zero)
		}else{
			format!("{:?}", data.state)
		}
	}
	...
	pub fn new(
		...
		Ok((SessionImpl {
			core: SessionCore {
				...
				nonce: params.nonce,
				completed,
				broadcast_signature: params.broadcast_signature
			},
	...
	pub fn on_partial_signature(&self, sender: &NodeId, message: &EcdsaPartialSignature) -> Result<(), Error> {
		...
		for node in data.consensus_session.consensus_non_rejected_nodes() {
			self.core.cluster.send(&node, Message::EcdsaSigning(EcdsaSigningMessage::EcdsaSigningSessionCompleted(EcdsaSigningSessionCompleted {
				...
				sub_session: self.core.access_key.clone().into(),
				session_nonce: self.core.nonce,
				signature: if self.core.broadcast_signature {
	...
	pub fn result(&self) -> Option<Result<Signature, Error>> {
