...
use key_server_cluster::decryption_session_with_payload::SessionImpl as DecryptionWithPayloadSession;
...
pub enum ContinueAction {
	...
	/// Decryption session + origin
	DecryptWithPayload(Arc<DecryptionWithPayloadSession>, Option<Address>),
		...
		if let Some(result) = core.result_computer.compute_result(threshold, confirmations, versions) {
			...
			if let (Some(continue_with), Err(error)) = (data.continue_with.as_ref(), result.as_ref()) {
				let origin = match *continue_with {
					...
					ContinueAction::DecryptWithPayload(_, origin) => origin.clone(),
					_ => None,
				};
				...
				let requester = match *continue_with {
					...
					ContinueAction::DecryptWithPayload(ref session, _) => session.requester().and_then(|r| r.address(&core.meta.id).ok()),
					_ => None,
				};
