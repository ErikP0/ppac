...
use super::{Error, SerializableH256, SerializableU256, SerializablePublic, SerializableSecret,
...
pub enum Message {
	...
	/// Decryption with payload message
	DecryptionWithPayload(DecryptionWithPayloadMessage),
...
/// All possible messages that can be sent during consensus establishing sending payload
#[derive(Clone, Debug, Serialize, Deserialize)]
pub enum ConsensusMessageWithPayload {
    /// Initialize consensus session.
    InitializeConsensusSession(InitializeConsensusSessionWithPayload),
    /// Confirm/reject consensus session initialization.
    ConfirmConsensusInitialization(ConfirmConsensusInitialization),
}
...
/// All possible messages that can be sent during a decryption session carrying authorization payload
#[derive(Clone, Debug)]
pub enum DecryptionWithPayloadMessage {
	/// Negotiate a key version of the signing key `traits::JOINT_SIGNATURE_KEY_ID`
	KeyVersionNegotiation(DecryptionWithPayloadKeyVersionNegotiation),
	/// Sign the log transaction using the previously negotiated signing key version
	SignLogTransaction(EcdsaSigningWithPayloadMessage),
	/// Consensus establishing message carrying additional payload
	DecryptionConsensusMessage(DecryptionConsensusMessage),
	/// Request partial decryption from node.
	RequestPartialDecryption(RequestProxyDecryption),
	/// Partial decryption is completed.
	PartialDecryption(ProxyDecryption),
	DecryptionWithPayloadCompleted(DecryptionSessionCompleted),
	/// When decryption session error has occured.
	DecryptionSessionError(DecryptionSessionError),
}
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub enum EcdsaSigningWithPayloadMessage {
	/// Consensus establishing message.
	EcdsaSigningConsensusMessage(EcdsaSigningConsensusWithPayloadMessage),
...
/// Node is asked to be part of consensus group providing additional payload
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct InitializeConsensusSessionWithPayload {
    /// Requester.
    pub requester: SerializableRequester,
    /// Key version.
    pub version: SerializableH256,
    /// Payload
    pub payload: Vec<SerializableU256>,
}
...
pub struct EcdsaSigningSessionCompleted {
	...
	/// Session-level nonce.
	pub session_nonce: u64,
	/// the jointly computed signature if broadcast is enabled
...
// /// Consensus-related decryption message carrying additional payload
// #[derive(Clone, Debug, Serialize, Deserialize)]
// pub struct DecryptionConsensusWithPayloadMessage {
//     /// Generation session Id.
//     pub session: MessageSessionId,
//     /// Signing session Id.
//     pub sub_session: SerializableSecret,
...
//     pub origin: Option<SerializableAddress>,
//     /// Consensus message.
//     pub message: ConsensusMessageWithPayload,
// }

/// Consensus-related ECDSA signing message carrying additional payload
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct EcdsaSigningConsensusWithPayloadMessage {
	/// Generation session Id.
	pub session: MessageSessionId,
	/// Signing session Id.
	pub sub_session: SerializableSecret,
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct DecryptionWithPayloadKeyVersionNegotiation {
	pub signing_key: KeyVersionNegotiationMessage,
	pub decryption_key: KeyVersionNegotiationMessage
}

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct RequestProxyDecryption {
	/// Encryption session Id.
	pub session: MessageSessionId,
	/// Decryption session Id.
	pub sub_session: SerializableSecret,
...
pub struct ProxyDecryption {
	...
	/// Session-level nonce.
	pub session_nonce: u64,
	/// ElGamal ciphertext 1: k * G
	pub common_point: SerializablePublic,
	/// ElGamal ciphertext 2: M + k * R
	pub encrypted_point: SerializablePublic,
	// verification proof data
	/// commitment to k
	pub k_commitment: SerializablePublic,
	/// commitment to share
	pub share_commitment: SerializablePublic,
	/// commitment to encrypted point
	pub encrypted_point_commitment: SerializablePublic,
	/// response to k
	pub k_response: SerializableSecret,
	/// response to share
	pub share_response: SerializableSecret,
}
...
impl Message {
	pub fn is_initialization_message(&self) -> bool {
		match *self {
			...
			Message::DecryptionWithPayload(DecryptionWithPayloadMessage::KeyVersionNegotiation(ref msg)) => match (&msg.decryption_key, &msg.signing_key) {
				(KeyVersionNegotiationMessage::RequestKeyVersions(_), KeyVersionNegotiationMessage::RequestKeyVersions(_)) => true,
				_ => false,
			},
	...
	pub fn is_error_message(&self) -> bool {
		match *self {
			...
			Message::DecryptionWithPayload(DecryptionWithPayloadMessage::DecryptionSessionError(_)) => true,
	...
	pub fn session_nonce(&self) -> Option<u64> {
		match *self {
			...
			Message::DecryptionWithPayload(ref message) => Some(message.session_nonce()),
...
impl DecryptionWithPayloadMessage {
	pub fn session_id(&self) -> &SessionId {
		match *self {
			DecryptionWithPayloadMessage::KeyVersionNegotiation(ref msg) => &msg.decryption_key.session_id(),
			DecryptionWithPayloadMessage::SignLogTransaction(ref msg) => &msg.session_id(),
			DecryptionWithPayloadMessage::DecryptionConsensusMessage(ref msg) => &msg.session,
			DecryptionWithPayloadMessage::RequestPartialDecryption(ref msg) => &msg.session,
			DecryptionWithPayloadMessage::PartialDecryption(ref msg) => &msg.session,
			DecryptionWithPayloadMessage::DecryptionWithPayloadCompleted(ref msg) => &msg.session,
			DecryptionWithPayloadMessage::DecryptionSessionError(ref msg) => &msg.session,
		}
	}

	pub fn sub_session_id(&self) -> &Secret {
		match *self {
			DecryptionWithPayloadMessage::KeyVersionNegotiation(ref msg) => &msg.decryption_key.sub_session_id(),
			DecryptionWithPayloadMessage::SignLogTransaction(ref msg) => &msg.sub_session_id(),
			DecryptionWithPayloadMessage::DecryptionConsensusMessage(ref msg) => &msg.sub_session,
			DecryptionWithPayloadMessage::RequestPartialDecryption(ref msg) => &msg.sub_session,
			DecryptionWithPayloadMessage::PartialDecryption(ref msg) => &msg.sub_session,
			DecryptionWithPayloadMessage::DecryptionWithPayloadCompleted(ref msg) => &msg.sub_session,
			DecryptionWithPayloadMessage::DecryptionSessionError(ref msg) => &msg.sub_session,
		}
	}

	pub fn session_nonce(&self) -> u64 {
		match *self {
			DecryptionWithPayloadMessage::KeyVersionNegotiation(ref msg) => msg.decryption_key.session_nonce(),
			DecryptionWithPayloadMessage::SignLogTransaction(ref msg) => msg.session_nonce(),
			DecryptionWithPayloadMessage::DecryptionConsensusMessage(ref msg) => msg.session_nonce,
			DecryptionWithPayloadMessage::RequestPartialDecryption(ref msg) => msg.session_nonce,
			DecryptionWithPayloadMessage::PartialDecryption(ref msg) => msg.session_nonce,
			DecryptionWithPayloadMessage::DecryptionWithPayloadCompleted(ref msg) => msg.session_nonce,
			DecryptionWithPayloadMessage::DecryptionSessionError(ref msg) => msg.session_nonce,
		}
	}
}
...
impl EcdsaSigningWithPayloadMessage {
	pub fn session_id(&self) -> &SessionId {
		match *self {
			EcdsaSigningWithPayloadMessage::EcdsaSigningConsensusMessage(ref msg) => &msg.session,
			EcdsaSigningWithPayloadMessage::EcdsaSignatureNonceGenerationMessage(ref msg) => &msg.session,
			EcdsaSigningWithPayloadMessage::EcdsaInversionNonceGenerationMessage(ref msg) => &msg.session,
			EcdsaSigningWithPayloadMessage::EcdsaInversionZeroGenerationMessage(ref msg) => &msg.session,
			EcdsaSigningWithPayloadMessage::EcdsaSigningInversedNonceCoeffShare(ref msg) => &msg.session,
			EcdsaSigningWithPayloadMessage::EcdsaRequestPartialSignature(ref msg) => &msg.session,
			EcdsaSigningWithPayloadMessage::EcdsaPartialSignature(ref msg) => &msg.session,
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionError(ref msg) => &msg.session,
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionCompleted(ref msg) => &msg.session,
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionDelegation(ref msg) => &msg.session,
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionDelegationCompleted(ref msg) => &msg.session,
		}
	}

	pub fn sub_session_id(&self) -> &Secret {
		match *self {
			EcdsaSigningWithPayloadMessage::EcdsaSigningConsensusMessage(ref msg) => &msg.sub_session,
			EcdsaSigningWithPayloadMessage::EcdsaSignatureNonceGenerationMessage(ref msg) => &msg.sub_session,
			EcdsaSigningWithPayloadMessage::EcdsaInversionNonceGenerationMessage(ref msg) => &msg.sub_session,
			EcdsaSigningWithPayloadMessage::EcdsaInversionZeroGenerationMessage(ref msg) => &msg.sub_session,
			EcdsaSigningWithPayloadMessage::EcdsaSigningInversedNonceCoeffShare(ref msg) => &msg.sub_session,
			EcdsaSigningWithPayloadMessage::EcdsaRequestPartialSignature(ref msg) => &msg.sub_session,
			EcdsaSigningWithPayloadMessage::EcdsaPartialSignature(ref msg) => &msg.sub_session,
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionError(ref msg) => &msg.sub_session,
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionCompleted(ref msg) => &msg.sub_session,
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionDelegation(ref msg) => &msg.sub_session,
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionDelegationCompleted(ref msg) => &msg.sub_session,
		}
	}

	pub fn session_nonce(&self) -> u64 {
		match *self {
			EcdsaSigningWithPayloadMessage::EcdsaSigningConsensusMessage(ref msg) => msg.session_nonce,
			EcdsaSigningWithPayloadMessage::EcdsaSignatureNonceGenerationMessage(ref msg) => msg.session_nonce,
			EcdsaSigningWithPayloadMessage::EcdsaInversionNonceGenerationMessage(ref msg) => msg.session_nonce,
			EcdsaSigningWithPayloadMessage::EcdsaInversionZeroGenerationMessage(ref msg) => msg.session_nonce,
			EcdsaSigningWithPayloadMessage::EcdsaSigningInversedNonceCoeffShare(ref msg) => msg.session_nonce,
			EcdsaSigningWithPayloadMessage::EcdsaRequestPartialSignature(ref msg) => msg.session_nonce,
			EcdsaSigningWithPayloadMessage::EcdsaPartialSignature(ref msg) => msg.session_nonce,
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionError(ref msg) => msg.session_nonce,
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionCompleted(ref msg) => msg.session_nonce,
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionDelegation(ref msg) => msg.session_nonce,
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionDelegationCompleted(ref msg) => msg.session_nonce,
		}
	}
}
...
impl fmt::Display for Message {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match *self {
			...
			Message::DecryptionWithPayload(ref message) => write!(f, "DecryptionWithPayload.{}", message),
...
impl fmt::Display for ConsensusMessageWithPayload {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ConsensusMessageWithPayload::InitializeConsensusSession(_) => {
                write!(f, "InitializeConsensusSession")
            }
            ConsensusMessageWithPayload::ConfirmConsensusInitialization(ref msg) => {
                write!(f, "ConfirmConsensusInitialization({})", msg.is_confirmed)
            }
        }
    }
}
...
impl fmt::Display for DecryptionWithPayloadMessage {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match *self {
			DecryptionWithPayloadMessage::KeyVersionNegotiation(ref msg) => write!(f, "KeyVersionNegotiation.(decryption={}, signing={})", msg.decryption_key, msg.signing_key),
			DecryptionWithPayloadMessage::SignLogTransaction(ref msg) => write!(f, "SignLogTransaction.{}", msg),
			DecryptionWithPayloadMessage::DecryptionConsensusMessage(ref m) => write!(f, "DecryptionConsensusMessageW.{}", m.message),
			DecryptionWithPayloadMessage::RequestPartialDecryption(_) => write!(f, "RequestPartialDecryption"),
			DecryptionWithPayloadMessage::PartialDecryption(_) => write!(f, "PartialDecryption"),
			DecryptionWithPayloadMessage::DecryptionWithPayloadCompleted(_) => write!(f, "DecryptionWithPayloadCompleted"),
			DecryptionWithPayloadMessage::DecryptionSessionError(_) => write!(f, "DecryptionSessionError"),
		}
	}
}
...
impl fmt::Display for EcdsaSigningWithPayloadMessage {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match *self {
			EcdsaSigningWithPayloadMessage::EcdsaSigningConsensusMessage(ref m) => write!(f, "EcdsaSigningConsensusMessage.{}", m.message),
			EcdsaSigningWithPayloadMessage::EcdsaSignatureNonceGenerationMessage(ref m) => write!(f, "EcdsaSignatureNonceGenerationMessage.{}", m.message),
			EcdsaSigningWithPayloadMessage::EcdsaInversionNonceGenerationMessage(ref m) => write!(f, "EcdsaInversionNonceGenerationMessage.{}", m.message),
			EcdsaSigningWithPayloadMessage::EcdsaInversionZeroGenerationMessage(ref m) => write!(f, "EcdsaInversionZeroGenerationMessage.{}", m.message),
			EcdsaSigningWithPayloadMessage::EcdsaSigningInversedNonceCoeffShare(_) => write!(f, "EcdsaSigningInversedNonceCoeffShare"),
			EcdsaSigningWithPayloadMessage::EcdsaRequestPartialSignature(_) => write!(f, "EcdsaRequestPartialSignature"),
			EcdsaSigningWithPayloadMessage::EcdsaPartialSignature(_) => write!(f, "EcdsaPartialSignature"),
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionError(_) => write!(f, "EcdsaSigningSessionError"),
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionCompleted(_) => write!(f, "EcdsaSigningSessionCompleted"),
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionDelegation(_) => write!(f, "EcdsaSigningSessionDelegation"),
			EcdsaSigningWithPayloadMessage::EcdsaSigningSessionDelegationCompleted(_) => write!(f, "EcdsaSigningSessionDelegationCompleted"),
		}
	}
}
