...
use key_server_cluster::decryption_session_with_payload::{
	...
	SessionImpl as DecryptionSessionWithPayload,
};
...
pub trait ClusterClient: Send + Sync {
    ...
    /// Start new decryption session.
    fn new_decryption_session_payload(
        &self,
        session_id: SessionId,
        requester: Requester,
        payload: Vec<U256>,
    ) -> Result<WaitableSession<DecryptionSessionWithPayload>, Error>;
...
pub fn new_network_cluster(
	...
	log_contract_address: Option<Address>,
	...
	let sessions = Arc::new(ClusterSessions::new(&config, servers_set_change_creator_connector.clone(), log_contract_address, eth_client));
...
pub fn new_test_cluster(
	...
	let servers_set_change_creator_connector = connection_trigger.servers_set_change_creator_connector();
	let mut sessions = ClusterSessions::new(&config, servers_set_change_creator_connector.clone(), Some(Address::default()), Arc::new(DummyEthClient::new()));
...
impl<C: ConnectionManager> ClusterClient for ClusterClientImpl<C> {
    ...
    fn new_decryption_session_payload(
        ...
        let session = self.data.sessions.decryption_sessions_payload.insert(
			cluster,
			self.data.self_key_pair.public().clone(),
			session_id.clone(),
			None,
			false,
			Some(DecryptionSessionWithPayloadCreationData {
				requester,
				payload
			}),
        )?;
        let initialization_result = session.session.initialize();
        process_initialization_result(
            initialization_result,
            session,
            &self.data.sessions.decryption_sessions_payload,
        )
    }
...
pub mod tests {
	...
	use key_server_cluster::client_sessions::decryption_session_with_payload::SessionImpl;
	...
	impl ClusterClient for DummyClusterClient {
		...
		fn new_decryption_session_payload(&self, _session_id: SessionId, _requester: Requester, _payload: Vec<U256>) -> Result<WaitableSession<SessionImpl>, Error> {
			unimplemented!("test-only")
		}
