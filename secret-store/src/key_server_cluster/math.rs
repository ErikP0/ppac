...
/// Computes the commitments to the coefficients of the polynomial that shares the distributed key
/// `node_publics`: vec of publics of each player
pub fn compute_coefficient_commitments<'a, I>(node_publics: I) -> Result<Vec<Public>, Error> where I: IntoIterator<Item=&'a Vec<Public>> {
	let mut node_publics = node_publics.into_iter();
	let mut coefficients = node_publics.next().expect("node_publics must at least contain the publics of selfnode").clone();
	for other_coefficients in node_publics {
		debug_assert_eq!(coefficients.len(), other_coefficients.len());
		for (i,c) in coefficients.iter_mut().enumerate() {
			ec_math_utils::public_add(c, &other_coefficients[i])?;
		}
	}
	Ok(coefficients)
}
...
pub struct PartialDecryptionShare {
	...
	pub common_point: Public,
	pub encrypted_point: Public,
	// non-interactive Sigma-protocol proof
	pub k_commitment: Public,
	pub share_commitment: Public,
	pub encrypted_point_commitment: Public,
	pub k_response: Secret,
	pub share_response: Secret,
}

/// compute hash over items in `iter`
pub fn compute_challenge<'a, I>(iter: I) -> Secret where I: Iterator<Item=&'a Public> {
	...
	hash.finalize(&mut hash_value);
	Secret::from(hash_value)
}

/// computes an ElGamal ciphertext as follows and returns (C1,C2)
/// pick random exponent k
/// C1 = k * G
/// C_2 = share * message + k * recipient_public
/// where G is the generator
pub fn compute_decryption_share(share: &Secret, message: &Public, recipient_public: &Public, public_knowledge: &Vec<Public>) -> Result<PartialDecryptionShare, Error>
{
	// compute an ElGamal ciphertext of share * message
	let k = generate_random_scalar()?;
	let random_point = {
		let mut g = ec_math_utils::generation_point();
		ec_math_utils::public_mul_secret(&mut g, &k)?;
		g
	};
	let encrypted_point = {
		let mut message = message.clone();
		ec_math_utils::public_mul_secret(&mut message, share)?;
		let mut b = recipient_public.clone();
		ec_math_utils::public_mul_secret(&mut b, &k)?;
		ec_math_utils::public_add(&mut b, &message)?;
		b
	};

	let k_t = generate_random_scalar()?;
	let share_t = generate_random_scalar()?;
	let k_commitment = compute_public_share(&k_t)?;
	let share_commitment = compute_public_share(&share_t)?;
	let encrypted_point_commitment = {
		let mut m = message.clone();
		ec_math_utils::public_mul_secret(&mut m, &share_t)?;
		let mut r = recipient_public.clone();
		ec_math_utils::public_mul_secret(&mut r, &k_t)?;
		ec_math_utils::public_add(&mut m, &r)?;
		m
	};
	let challenge = compute_challenge(vec![&k_commitment, &share_commitment, &encrypted_point_commitment, &random_point, &encrypted_point].into_iter().chain(public_knowledge.iter()));
	let k_response = {
		let mut k_response = k.clone();
		k_response.mul(&challenge)?;
		k_response.add(&k_t)?;
		k_response
	};
	let share_response = {
		let mut share_response = share.clone();
		share_response.mul(&challenge)?;
		share_response.add(&share_t)?;
		share_response
	};

	Ok(PartialDecryptionShare {
		common_point: random_point,
		encrypted_point,
		k_commitment,
		share_commitment,
		encrypted_point_commitment,
		k_response,
		share_response
	})
}

pub fn compute_share_commitment(coefficient_commitments: &Vec<Public>, id: &Secret) -> Result<Public, Error> {
	// polynomial using coefficient_commitments
	assert!(coefficient_commitments.len() > 0);
	let mut sum = coefficient_commitments[0].clone();
	for (i, coeff) in coefficient_commitments.iter().enumerate().skip(1) {
		let mut id_pow = id.clone();
		id_pow.pow(i)?;
		let mut coeff = coeff.clone();
		ec_math_utils::public_mul_secret(&mut coeff, &id_pow)?;
		ec_math_utils::public_add(&mut sum, &coeff)?;
	}
	Ok(sum)
}

pub fn verify_decryption_share(
	coefficient_commitments: &Vec<Public>,
	id: &Secret,
	common_point: &Public,
	encrypted_point: &Public,
	recipient_public: &Public,
	nonce: &Secret,
	partial_decryption_share: &PartialDecryptionShare
) -> Result<bool, Error> {
	let public_share_commitment = compute_share_commitment(coefficient_commitments, id)?;
	let nonce_point = compute_public_share(nonce)?;
	let challenge = compute_challenge(
		vec![partial_decryption_share.k_commitment, partial_decryption_share.share_commitment, partial_decryption_share.encrypted_point_commitment, partial_decryption_share.common_point, partial_decryption_share.encrypted_point].iter()
			.chain(vec![common_point, encrypted_point, &public_share_commitment, recipient_public, &nonce_point])
	);

	// verify partial_decryption_share.common_point
	{
		let mut left = ec_math_utils::generation_point();
		ec_math_utils::public_mul_secret(&mut left, &partial_decryption_share.k_response)?;

		let mut right = partial_decryption_share.common_point.clone();
		ec_math_utils::public_mul_secret(&mut right, &challenge)?;
		ec_math_utils::public_add(&mut right, &partial_decryption_share.k_commitment)?;
	...
	// verify that the share was used
	{
		let mut left = ec_math_utils::generation_point();
		ec_math_utils::public_mul_secret(&mut left, &partial_decryption_share.share_response)?;

		let mut right = public_share_commitment;
		ec_math_utils::public_mul_secret(&mut right, &challenge)?;
		ec_math_utils::public_add(&mut right, &partial_decryption_share.share_commitment)?;
	...
	// verify correct ciphertext
	{
		let mut left = common_point.clone();
		ec_math_utils::public_mul_secret(&mut left, &partial_decryption_share.share_response)?;
		let mut left2 = recipient_public.clone();
		ec_math_utils::public_mul_secret(&mut left2, &partial_decryption_share.k_response)?;
		ec_math_utils::public_add(&mut left, &left2)?;

		let mut right = partial_decryption_share.encrypted_point.clone();
		ec_math_utils::public_mul_secret(&mut right, &challenge)?;
		ec_math_utils::public_add(&mut right, &partial_decryption_share.encrypted_point_commitment)?;
...
#[test]
fn test_compute_decryption_share() {
	let share = generate_random_scalar().unwrap();
	let message = generate_random_point().unwrap();

	let pk = vec![];

	let keypair = Random.generate();
	let decryption_share = compute_decryption_share(&share, &message, keypair.public(), &pk).unwrap();
	let mut c1 = decryption_share.common_point;
	let c2 = decryption_share.encrypted_point;

	// try decryption
	ec_math_utils::public_mul_secret(&mut c1, keypair.secret()).unwrap();
	let mut res = c2.clone();
	ec_math_utils::public_sub(&mut res, &c1).unwrap();

	let mut cmp = message.clone();
	ec_math_utils::public_mul_secret(&mut cmp, &share).unwrap();
	assert_eq!(cmp, res);
}

fn one() -> Secret {
	let mut n = Secret::zero();
	n.dec().unwrap();
	n.neg().unwrap();
	n
}

fn lagrange_interpolator<'a, I: Iterator<Item=&'a Secret>>(self_value: &Secret, iter: I) -> Result<Secret, Error> {
	let mut prod = one();
	let other_values = iter.filter(|v| &self_value != v);
	for xj in other_values {
		let mut xj_neg = xj.clone();
		xj_neg.neg()?;
		let mut xi = self_value.clone();
		xi.sub(xj)?;
		invert_secret(&mut xi)?;
...
#[test]
fn test_lagrange() {
	let poly = generate_random_polynom(3).unwrap();
	assert_eq!(poly.len(), 4);
	let ids: Vec<_> = (0..5).map(|_| generate_random_scalar().unwrap()).collect();
	let shares: Vec<_> = ids.iter().map(|id| compute_polynom(&poly, id).unwrap())
		.collect();

	// reconstruct
	let mut sum = Secret::zero();
	for (id,share) in ids.iter().zip(shares.iter()) {
		let mut l = lagrange_interpolator(id, ids.iter()).unwrap();
		l.mul(share).unwrap();
		sum.add(&l).unwrap();
	}
	assert_eq!(sum, poly[0]);
}

pub fn aggregate_decryption_shares(decryption_shares: Vec<(Secret, Public, Public)>, original_c2: &Public) -> Result<(Public,Public), Error> {
	let all_values = || decryption_shares.iter().map(|(other_id,_,_)| other_id);
	let interpolators = decryption_shares.iter()
		.map(|(id,_,_)| lagrange_interpolator(id, all_values()))
		.collect::<Result<Vec<Secret>, Error>>()?;
	let c_1_prime = decryption_shares.iter().zip(interpolators.iter())
		.map(|((_, c1, _), l)| {
			let mut c1 = c1.clone();
			ec_math_utils::public_mul_secret(&mut c1, l)?;
			Ok(c1)
		})
		.collect::<Result<Vec<Public>, Error>>()?;
	let mut c_1_prime = compute_public_sum(c_1_prime.iter())?;
	ec_math_utils::public_negate(&mut c_1_prime)?;
	let c_2_sum = decryption_shares.iter().zip(interpolators.iter())
		.map(|((_, _, c2), l)| {
			let mut c2 = c2.clone();
			ec_math_utils::public_mul_secret(&mut c2, l)?;
			Ok(c2)
		})
		.collect::<Result<Vec<Public>, Error>>()?;
	let c_2_sum = compute_public_sum(c_2_sum.iter())?;
	let mut c_2_prime = original_c2.clone();
	ec_math_utils::public_sub(&mut c_2_prime, &c_2_sum)?;
	Ok((c_1_prime, c_2_prime))
}
...
pub mod tests {
	...
	#[test]
	fn threshold_proxy_decryption() {
		let test_cases = vec![(0,1), (1, 3), (2, 5), (2, 7), (2, 7), (3, 8)];
		for (t, n) in test_cases {
			// generate two shared secrets
			let artifacts = run_key_generation(t, n, None, None);

			// choose random message
			let m = generate_random_point().unwrap();
			// encrypt with generated public key
			let (c1,c2) = {
				let k = generate_random_scalar().unwrap();
				let mut c1 = ec_math_utils::generation_point();
				ec_math_utils::public_mul_secret(&mut c1, &k).unwrap();
				let mut c2 = artifacts.joint_public.clone();
				ec_math_utils::public_mul_secret(&mut c2, &k).unwrap();
				ec_math_utils::public_add(&mut c2, &m).unwrap();
				(c1,c2)
			};

			// choose recipient key pair
			let recipient = Random.generate();
			let nonce = generate_random_scalar().unwrap();
			let nonce_point = compute_public_share(&nonce).unwrap();

			// commitments to coefficients of each polynomial
			let public_commitments: Vec<_> = artifacts.polynoms1.iter()
				.map(|poly| poly.iter().map(compute_public_share).collect::<Result<Vec<_>,Error>>().unwrap())
				.collect();
			let coefficient_commitments = compute_coefficient_commitments(&public_commitments).unwrap();

			let decryption_shares: Vec<_> = artifacts.secret_shares.iter().zip(&artifacts.id_numbers)
				.map(|(share,id)| {
					let share_commitment = compute_share_commitment(&coefficient_commitments, id).unwrap();
					let public_knowledge = vec![c1.clone(), c2.clone(), share_commitment, recipient.public().clone(), nonce_point.clone()];
					compute_decryption_share(share, &c1, recipient.public(), &public_knowledge).unwrap()
				})
				.collect();

			// check that verification works
			for (partial_dec_share, id) in decryption_shares.iter().zip(&artifacts.id_numbers) {
				let res = verify_decryption_share(&coefficient_commitments, id, &c1, &c2, recipient.public(), &nonce, partial_dec_share);
				assert_eq!(Ok(true), res);
			}

			// check that correctly encrypted
			{
				for (partial_dec_share, share) in decryption_shares.iter().zip(artifacts.secret_shares.iter()) {
					let mut d1 = partial_dec_share.common_point.clone();
					ec_math_utils::public_mul_secret(&mut d1, recipient.secret()).unwrap();
					let mut d2 = partial_dec_share.encrypted_point.clone();
					ec_math_utils::public_sub(&mut d2, &d1).unwrap();

					let mut cmp = c1.clone();
					ec_math_utils::public_mul_secret(&mut cmp, share).unwrap();
					assert_eq!(cmp, d2);
				}
			}
			...
			let aggregate = decryption_shares.into_iter().zip(artifacts.id_numbers)
				.take(t+1)
				.map(|(dec_share, id)|(id,dec_share.common_point,dec_share.encrypted_point))
				.collect();
			// the proxy-encrypted message
			let (mut d1,mut d2) = aggregate_decryption_shares(aggregate, &c2).unwrap();

			// decrypt
			ec_math_utils::public_mul_secret(&mut d1, recipient.secret()).unwrap();
			ec_math_utils::public_sub(&mut d2, &d1).unwrap();

			assert_eq!(m, d2, "Case t={}, n={}", t, n);
		}
	}

	#[test]
	fn decryption_share_verification_fails_for_invalid_share() {
		let poly = generate_random_polynom(5).unwrap();

		let coefficients: Vec<_> = poly.iter()
			.map(|coeff| compute_public_share(coeff).unwrap())
			.collect();
		let id = generate_random_scalar().unwrap();

		let share = compute_polynom(&poly, &id).unwrap();
		let c1 = generate_random_point().unwrap();
		let c2 = generate_random_point().unwrap();
		let share_commitment = compute_share_commitment(&coefficients, &id).unwrap();

		let recipient = generate_random_point().unwrap();
		let nonce = generate_random_scalar().unwrap();
		let nonce_point = compute_public_share(&nonce).unwrap();

		let public_knowledge = vec![c1.clone(), c2.clone(), share_commitment, recipient.clone(), nonce_point.clone()];

		// decryption share without using the secret share
		{
			let arbitrary_share = generate_random_scalar().unwrap();
			let decryption_share = compute_decryption_share(&arbitrary_share, &c1, &recipient, &public_knowledge).unwrap();
			let res = verify_decryption_share(&coefficients, &id, &c1, &c2, &recipient, &nonce, &decryption_share);
			assert_eq!(Ok(false), res);
		}

		// decryption share without using c1
		{
			let arbitrary_c1 = generate_random_point().unwrap();
			let decryption_share = compute_decryption_share(&share, &arbitrary_c1, &recipient, &public_knowledge).unwrap();
			let res = verify_decryption_share(&coefficients, &id, &c1, &c2, &recipient, &nonce,&decryption_share);
			assert_eq!(Ok(false), res);
		}

		// decryption share without using recipient public
		{
			let other_recipient = generate_random_point().unwrap();
			let decryption_share = compute_decryption_share(&share, &c1, &other_recipient, &public_knowledge).unwrap();
			let res = verify_decryption_share(&coefficients, &id, &c1, &c2, &recipient, &nonce,&decryption_share);
			assert_eq!(Ok(false), res);
		}

		// decryption share without the same public knowledge
		{
			let other_pk = vec![c1.clone(), generate_random_point().unwrap(), share_commitment, recipient.clone(), nonce_point];
			let decryption_share = compute_decryption_share(&share, &c1, &recipient, &other_pk).unwrap();
			let res = verify_decryption_share(&coefficients, &id, &c1, &c2, &recipient, &nonce, &decryption_share);
			assert_eq!(Ok(false), res);
		}

		// decryption share without the same nonce
		{
			let other_nonce = generate_random_scalar().unwrap();
			let decryption_share = compute_decryption_share(&share, &c1, &recipient, &public_knowledge).unwrap();
			let res = verify_decryption_share(&coefficients, &id, &c1, &c2, &recipient, &other_nonce, &decryption_share);
			assert_eq!(Ok(false), res);
		}
