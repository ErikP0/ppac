...
pub struct DocumentKeyShareVersion {
	...
	/// commitments to the share polynomial, if available
...
struct SerializableDocumentKeyShareVersionV3 {
	...
	/// commitments to the share polynomial, if available
	pub coefficient_commitments: Option<Vec<SerializablePublic>>,
}
...
impl DocumentKeyShareVersion {
	/// Create new version
	pub fn new(id_numbers: BTreeMap<NodeId, Secret>, secret_share: Secret, coefficient_commitments: Option<Vec<Public>>) -> Self {
...
impl From<DocumentKeyShareVersion> for SerializableDocumentKeyShareVersionV3 {
	fn from(version: DocumentKeyShareVersion) -> Self {
		SerializableDocumentKeyShareVersionV3 {
			...
			coefficient_commitments: version.coefficient_commitments.map(|cs| cs.into_iter().map(Into::into).collect()),
		}
	}
}
		...
		DocumentKeyShare {
			...
			versions: key.versions.into_iter()
				.map(|v| DocumentKeyShareVersion {
					...
					coefficient_commitments: v.coefficient_commitments.map(|cs| cs.into_iter().map(Into::into).collect()),
				})
				.collect(),
		}
	}
