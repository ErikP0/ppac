...
use blockchain::{SigningKeyPair, EthClient};
use key_server_cluster::{math, new_network_cluster, ClusterSession, WaitableSession};
use traits::{AdminSessionsServer, ServerKeyGenerator, DocumentKeyServer, MessageSigner, KeyServer, JOINT_SIGNATURE_KEY_ID};
...
impl KeyServerImpl {
	...
	pub fn new(config: &ClusterConfiguration, key_server_set: Arc<dyn KeyServerSet>, self_key_pair: Arc<dyn SigningKeyPair>,
		acl_storage: Arc<dyn AclStorage>, key_storage: Arc<dyn KeyStorage>, log_contract_address: Option<Address>, eth_client: Arc<dyn EthClient>, executor: Executor) -> Result<Self, Error>
	{
		Ok(KeyServerImpl {
			data: Arc::new(Mutex::new(KeyServerCore::new(config, key_server_set, self_key_pair, acl_storage, key_storage, log_contract_address, eth_client, executor)?)),
		})
	}
...
impl ServerKeyGenerator for KeyServerImpl {
	...
	fn generate_joint_signature_key(&self) -> Box<dyn Future<Item=Public, Error=Error> + Send> {
		let (storage_res, required_threshold) = {
			let guard = self.data.lock();
			let storage = &guard.cluster.cluster_configuration().key_storage;
			let required_threshold = (guard.config.nodes.len() -1)/2;

			(storage.get(&JOINT_SIGNATURE_KEY_ID), required_threshold)
		};
		// we need to unlock self.data, otherwise a deadlock might occur when calling self.generate_key
		match storage_res {
			Ok(Some(ks)) if ks.threshold <= required_threshold => {
				Box::new(ok(ks.public.clone()))
			},
			Ok(Some(_))  => {
				Box::new(err(Error::Internal("previously generated joint signature key has insufficient threshold".into())))
			},
			Ok(None) => {
				let requester = Requester::Public(self.data.lock().cluster.cluster_configuration().self_key_pair.public().clone());
				self.generate_key(JOINT_SIGNATURE_KEY_ID.clone(), requester, required_threshold)
			},
...
impl DocumentKeyServer for KeyServerImpl {
    ...
    fn restore_document_key_shadow_payload(
        ...
        key_id: ServerKeyId,
        requester: Requester,
        payload: Vec<ethereum_types::U256>,
    ) -> Box<dyn Future<Item = ProxyEncryptedDocumentKey, Error = Error> + Send> {
        return_session(self.data.lock().cluster.new_decryption_session_payload(
            key_id,
            requester.clone(),
            payload,
        ))
...
impl KeyServerCore {
		...
		acl_storage: Arc<dyn AclStorage>, key_storage: Arc<dyn KeyStorage>, log_contract_address: Option<Address>, eth_client: Arc<dyn EthClient>, executor: Executor) -> Result<Self, Error>
	{
		...
		let core = new_network_cluster(executor, cconfig, net_config, log_contract_address, eth_client)?;
...
pub mod tests {
	...
	use key_server_cluster::{math, ClusterClient};
	...
	impl DocumentKeyServer for DummyKeyServer {
		...
		fn restore_document_key_shadow_payload(&self, _key_id: ServerKeyId, _requester: Requester, _payload: Vec<U256>) -> Box<dyn Future<Item=ProxyEncryptedDocumentKey, Error=Error> + Send> {
			unimplemented!("test-only")
		}
	}
	...
	impl AsRef<dyn KeyServer> for KeyServerImpl {
		fn as_ref(&self) -> &(dyn KeyServer + 'static) {
			self
		}
	}

	pub fn wait_until_fully_connected<KS>(key_servers: &Vec<KS>) where KS : AsRef<dyn KeyServer> {
		// wait until connections are established. It is fast => do not bother with events here
		let start = time::Instant::now();
		let mut tried_reconnections = false;
		loop {
			if key_servers.iter().all(|ks| ks.as_ref().cluster().is_fully_connected()) {
				break;
			}
			...
			for key_server in key_servers {
				if !key_server.as_ref().cluster().is_fully_connected() {
					fully_connected = false;
					if !old_tried_reconnections {
						tried_reconnections = true;
						key_server.as_ref().cluster().connect();
					}
				}
			}
			if fully_connected {
	...
	fn make_key_servers(start_port: u16, num_nodes: usize) -> (Vec<KeyServerImpl>, Vec<Arc<DummyKeyStorage>>, Runtime) {
		...
		let key_servers: Vec<_> = configs.into_iter().enumerate().map(|(i, cfg)|
			KeyServerImpl::new(&cfg, Arc::new(MapKeyServerSet::new(false, key_servers_set.clone())),
				...
				key_storages[i].clone(),  Some(Address::default()), eth_client.clone(), runtime.executor()).unwrap()
		).collect();

		wait_until_fully_connected(&key_servers);
	...
	#[test]
	fn shadow_decryption_session_works_over_network_with_3_nodes() {
		let (key_servers, key_storages, runtime) = make_key_servers(6121, 3);
		// generate signing key
		key_servers[0].generate_joint_signature_key().wait().unwrap();
		...
		let generated_document_key = crypto::publickey::ecies::decrypt(&secret, &DEFAULT_MAC, &generated_key).unwrap();
		let generated_document_key = Public::from_slice(&generated_document_key);

		assert!(key_storages.iter().all(|ks| {
			let key_share = ks.get(&document).unwrap().unwrap();
			key_share.encrypted_point.is_some() && key_share.common_point.is_some()
		}));

		let x = key_servers[0].restore_document_key_shadow_payload(*document, signature.clone(), vec![]);
		let x = x.wait();
		let proxy_encrypted_document_key = x.unwrap();
		let decrypted_document_key = decrypt(&secret, proxy_encrypted_document_key);
		assert_eq!(generated_document_key, decrypted_document_key);
		drop(runtime)
	}

	#[test]
	fn generate_joint_signature_key_works_over_network_with_3_nodes() {
		let _ = ::env_logger::try_init();
		let (key_servers, _, runtime) = make_key_servers(6124, 3);

		// generate joint signature key
		let joint_signature_key = key_servers[0].generate_joint_signature_key().wait().unwrap();
		// returned key is the joint signature key
		assert_eq!(Ok(Some(joint_signature_key)), key_servers[0].data.lock().cluster.cluster_configuration().key_storage.get(&JOINT_SIGNATURE_KEY_ID).map(|ks| ks.map(|x|x.public)));
		// expect threshold 1
		assert_eq!(Ok(Some(1)), key_servers[0].data.lock().cluster.cluster_configuration().key_storage.get(&JOINT_SIGNATURE_KEY_ID).map(|ks| ks.map(|x|x.threshold)));

		// multiple calls return the previously generated key
		let other_key = key_servers[1].generate_joint_signature_key().wait().unwrap();
		assert_eq!(joint_signature_key, other_key);
		drop(runtime);
	}
}
