...
pub struct Configuration {
	...
	pub self_http_rpc_interface: String,
	pub self_http_rpc_port: u16
}
	...
	impl KeyServer {
		...
		pub fn new(mut conf: Configuration, deps: Dependencies, executor: Executor) -> Result<Self, String> {
			...
			let mut cconf = parity_secretstore::ServiceConfiguration {
				...
				cors: conf.cors,
				http_rpc_address: parity_secretstore::NodeAddress {
					address: conf.self_http_rpc_interface.clone(),
					port: conf.self_http_rpc_port
				}
			};
...
impl Default for Configuration {
	fn default() -> Self {
		...
		Configuration {
			...
			cors: Some(vec![]),
			self_http_rpc_interface: "127.0.0.1".to_owned(),
			self_http_rpc_port: 8545
		}
	}
}
